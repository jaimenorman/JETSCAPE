...
class PlotResults(common_base.CommonBase):
    ...
    def init_observable(self, observable_type, observable, block, centrality, pt_suffix='', self_normalize=False):
        ...
        # Initialize JETSCAPE
        self.initialize_jetscape(observable_type, observable, block, centrality, pt_suffix=pt_suffix, self_normalize=self_normalize, reference=False) # pre
        self.initialize_jetscape(observable_type, observable, block, centrality, pt_suffix=pt_suffix, self_normalize=self_normalize, reference=True)  # post
        
    #-------------------------------------------------------------------------------------------
    # Initialize JETSCAPE
    #-------------------------------------------------------------------------------------------
    def initialize_jetscape(self, observable_type, observable, block, centrality, pt_suffix='', self_normalize=False, reference=False):
        ...
        if self.observable_settings[jetscape_key]:
            n_events = self.input_file.Get('h_n_events').GetBinContent(1)
            self.observable_settings[jetscape_key].Scale(1./n_events)
            
            if self.sqrts == 200:
                if observable_type == 'hadron':
                    if observable == 'pt_ch_star':
                        self.observable_settings[jetscape_key].Scale(1./(2*self.eta_cut))
                        self.observable_settings[jetscape_key].Scale(1./(2*np.pi))
                    ...
                    if observable in ['IAA_star', 'dphi_star']:
                        self.observable_settings[jetscape_key].Scale(n_events)
            ...
            if self.sqrts == 2760:
                if observable_type == 'hadron':
                    if observable == 'pt_ch_alice':
                        sigma_inel = 0.0618
                        self.observable_settings[jetscape_key].Scale(1./(2*self.eta_cut))
                        self.observable_settings[jetscape_key].Scale(1./sigma_inel)
                        ...
                        self.observable_settings[jetscape_key].Scale(1./(2*self.eta_cut))
                        self.observable_settings[jetscape_key].Scale(1./(2*np.pi))
                        self.observable_settings[jetscape_key].Scale(1.e3) # convert to mb
                    elif observable == 'pt_ch_cms':
                        L_int = 230. # (in nb)
                        self.observable_settings[jetscape_key].Scale(1./(2*self.eta_cut))
                        self.observable_settings[jetscape_key].Scale(1./(2*np.pi))
                        self.observable_settings[jetscape_key].Scale(L_int)
                    ...
                    if observable == 'pt_atlas':
                        self.observable_settings[jetscape_key].Scale(1./(2*self.eta_cut))
                        self.observable_settings[jetscape_key].Scale(1.e9) # convert to nb
                    ...
                    if observable in ['Dz_atlas', 'Dpt_atlas']:
                        ...
                        if n_jets > 0.:
                            self.observable_settings[jetscape_key].Scale(1.*n_events/n_jets)
                        else:
            ...
            if self.sqrts == 5020:
                if observable_type == 'hadron':
                    if observable in ['pt_ch_alice', 'pt_pi_alice']:
                        ...
                        self.observable_settings[jetscape_key].Scale(1./(2*self.eta_cut))
                        self.observable_settings[jetscape_key].Scale(1./(2*np.pi))
                        self.observable_settings[jetscape_key].Scale(1./sigma)
                elif observable_type == 'inclusive_jet':
                    if observable == 'pt_alice':
                        self.observable_settings[jetscape_key].Scale(1./(2*self.eta_cut))
                        self.observable_settings[jetscape_key].Scale(1.e3) # convert to mb
                    if observable == 'pt_atlas':
                        self.observable_settings[jetscape_key].Scale(1./(2*self.y_cut))
                        self.observable_settings[jetscape_key].Scale(1.e9) # convert to nb
                    if observable == 'pt_cms':
                        self.observable_settings[jetscape_key].Scale(1./(2*self.eta_cut))
                        self.observable_settings[jetscape_key].Scale(1.e9) # convert to nb
            ...
            if self.scale_by:
                nBins = self.observable_settings[jetscape_key].GetNbinsX()
                for bin in range(1, nBins+1):
                    h_x = self.observable_settings[jetscape_key].GetBinCenter(bin)
                    h_y = self.observable_settings[jetscape_key].GetBinContent(bin)
                    h_error = self.observable_settings[jetscape_key].GetBinError(bin)
                    if self.scale_by == '1/pt':
                        scale_factor = 1./h_x
                    self.observable_settings[jetscape_key].SetBinContent(bin, h_y*scale_factor)
                    self.observable_settings[jetscape_key].SetBinError(bin, h_error*scale_factor)

            # Bin width
            self.observable_settings[jetscape_key].Scale(1., 'width')
            ...
            if self_normalize:
                if observable in ['zg', 'theta_g']:
                    ...
                    min_bin = 1
                nbins = self.observable_settings[jetscape_key].GetNbinsX()
                integral = self.observable_settings[jetscape_key].Integral(min_bin, nbins, 'width')
                if integral > 0.:
                    self.observable_settings[jetscape_key].Scale(1./integral)
                else:
                    print('WARNING: integral for self-normalization is 0')
                    
            if observable == 'nsubjettiness_alice':
                print(self.observable_settings[jetscape_key].Integral())
                
        # Form ratio of JETSCAPE to data
        if self.observable_settings['data_distribution'] and self.observable_settings[jetscape_key] and not self.observable_settings[jetscape_key].InheritsFrom(ROOT.TH2.Class()) and not self.skip_pp_ratio:
            self.observable_settings[f'ratio{jetscape_key}'] = self.plot_utils.divide_histogram_by_tgraph(self.observable_settings[jetscape_key],
                                                                              self.observable_settings['data_distribution'])
                                                                              
        else:
            self.observable_settings[f'ratio{jetscape_key}'] = None
    ...
    def plot_distribution_and_ratio(self, observable_type, observable, centrality, pt_suffix='', logy = False):
        ...
        if not self.observable_settings['jetscape_distribution'] or not self.observable_settings['jetscape_distribution_reference']:
            return
        ...
        legend = ROOT.TLegend(0.58,0.62,0.75,0.8)
        ...
        self.observable_settings['jetscape_distribution_reference'].SetFillStyle(1001)
        ...
        # Draw ratio
        pad2.cd()
        if self.observable_settings['ratiojetscape_distribution']:
            self.observable_settings['ratiojetscape_distribution'].SetFillColor(self.jetscape_color)
            self.observable_settings['ratiojetscape_distribution'].SetFillColorAlpha(self.jetscape_color, self.alpha)
            self.observable_settings['ratiojetscape_distribution'].SetFillStyle(1001)
            self.observable_settings['ratiojetscape_distribution'].SetMarkerSize(0.)
            self.observable_settings['ratiojetscape_distribution'].SetMarkerStyle(0)
            self.observable_settings['ratiojetscape_distribution'].SetLineWidth(0)
            self.observable_settings['ratiojetscape_distribution'].Draw('E3 same')
            
        if self.observable_settings['ratiojetscape_distribution_reference']:
            self.observable_settings['ratiojetscape_distribution_reference'].SetFillColor(self.jetscape_color_reference)
            self.observable_settings['ratiojetscape_distribution_reference'].SetFillColorAlpha(self.jetscape_color_reference, self.alpha)
            self.observable_settings['ratiojetscape_distribution_reference'].SetFillStyle(1001)
            self.observable_settings['ratiojetscape_distribution_reference'].SetMarkerSize(0.)
            self.observable_settings['ratiojetscape_distribution_reference'].SetMarkerStyle(0)
            self.observable_settings['ratiojetscape_distribution_reference'].SetLineWidth(0)
            self.observable_settings['ratiojetscape_distribution_reference'].Draw('E3 same')
...
if __name__ == '__main__':
    print('Executing plot_results.py...')
    print('')
